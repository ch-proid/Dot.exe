...
- 좌측 상단 `프로토콜 P-02 00:43 ▼`
- 터치 시 프로토콜 목표/예상 위협/현재 준비 상세 펼침
- 세포는 흰색 4×4 정사각 픽셀
- 영양분은 더 작은 초록색 2×2 정사각 픽셀
- 세포와 영양 모두 연속 좌표 기반의 부드러운 자유 이동
- CRT 곡률/가장자리 왜곡/비네팅 강화
- 기존보다 전체 밝기 소폭 상향
- 약 8~14초 간격의 짧은 디지털 지직 연출

다음 UI 작업은 이 파일을 사용자가 브라우저에서 검토한 뒤 피드백을 반영하는 것이다.
...
이 교훈을 문서화하기 직전/직후 동일한 유형의 판단 오류가 한 번 더 발생했다.

최신 B-1 UI를 GitHub에 올려 달라는 요청에서 다시 sandbox의 `gh` 부재를 확인한 뒤 “현재 GitHub 쓰기 도구가 없다”고 잘못 답했다. 사용자가 이전 교훈을 상기시키며 재확인을 요청했고, 이후 GitHub Connector의 branch/profile 조회가 정상 성공했다.

즉 이 오류는 한 번의 우연한 실수가 아니라 **도구 경로를 잘못 구분하면 반복될 수 있는 패턴**으로 확인되었다.

추가 재발 방지 규칙:

- GitHub 관련 판단은 반드시 Connector read 호출 결과를 근거로 한다.
- CLI 확인은 Connector 가능 여부 판단에 사용하지 않는다.
- 이전 세션/같은 대화에서 Connector 사용 이력이 있으면 먼저 Connector가 살아 있는지 재조회한다.
- 사용자가 “연결되어 있다”고 알려 준 경우, 불가능하다고 답하기 전에 실제 Connector 호출을 한 번 수행한다.
...
GitHub에 게시된 `Game/Design/UI/Dotexe_UI_B1_refined.html`을 사용자가 검토하고, B-1 UI의 다음 수정사항을 확정한다.
...
이 UI가 승인되면 같은 디자인 언어로 연구 / 브리핑 / 메일 / 분석 / 시스템 / 게임 오버 화면까지 확장한다.
